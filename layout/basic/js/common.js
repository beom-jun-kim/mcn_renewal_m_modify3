...
$(document).ready(function () {
   ...
   *  메인카테고리 toggle
   */
  $(".xans-product-listmain h2").toggle(
    function () {
      $(this).css(
        "background-image",
        'url("//img.echosting.cafe24.com/skin/mobile_ko_KR/layout/bg_title_open.gif")'
      );
      $(this).siblings().hide();
    ...
    function () {
      $(this).css(
        "background-image",
        'url("//img.echosting.cafe24.com/skin/mobile_ko_KR/layout/bg_title_close.gif")'
      );
      $(this).siblings().show();
  ...
  var globalBuyBtnScrollFunc = function () {
    // 구매버튼 관련변수
    var sFixId =
        $("#orderFixItem").size() > 0 ? "orderFixItem" : "fixedActionButton",
      $area = $("#orderFixArea"),
...
var globalLayerCloseFunc = function () {
  ...
  if (window.parent === window) self.clse();
  else {
    parent
      .$("html, body")
...
$("#wrap #container").css("height", "auto");


$('#header .search button').click(function(){
    $('.xans-layout-slidepackage').removeClass('active');
    $('.toggleBtn span').removeClass('active');
});

$('.xans-search-form .inner select option:first-child').text('기준선택');